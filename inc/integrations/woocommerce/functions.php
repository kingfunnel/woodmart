...
if ( ! function_exists( 'woodmart_update_cart_fragments_fix' ) ) {
	...
	function woodmart_update_cart_fragments_fix() {
		if ( ! apply_filters( 'woodmart_update_fragments_fix', false ) ) {
			return;
		}

		wp_enqueue_script( 'wd-update-cart-fragments-fix', WOODMART_SCRIPTS . '/scripts/wc/updateCartFragmentsFix.js',
			array( 'jquery', 'js-cookie' ), woodmart_get_theme_info( 'Version' ), true );
	}
...
if ( ! function_exists( 'woodmart_get_grid_el_class_new' ) ) {
	function woodmart_get_grid_el_class_new( $loop = 0, $different_sizes = false, $desktop_columns = 3, $tablet_columns = 4, $mobile_columns = false ) {
		...
		$sizes = array(
			array(
				'name'  => 'col-lg',
				'value' => woodmart_get_grid_el_columns( $desktop_columns ),
			),
			array(
				'name'  => 'col-md',
				'value' => woodmart_get_grid_el_columns( $tablet_columns ),
			),
			array(
				'name'  => 'col',
				'value' => $mobile_columns ? woodmart_get_grid_el_columns( $mobile_columns ) : woodmart_get_grid_el_columns( $tablet_columns ),
			),
		);
		...
		if ( apply_filters( 'woodmart_old_product_grid_classes', false ) ) {
			$result_sizes = array(
				'col-lg' => $desktop_columns,
				'col-md' => $tablet_columns,
				'col'    => $mobile_columns,
			);
		}
